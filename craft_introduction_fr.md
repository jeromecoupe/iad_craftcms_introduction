...
- La version "Team" à 279 USD vous offre un seul groupe d'utilisateurs et jusqu'à 5 comptes.
- Une version "Pro" à 399 USD vous permet de disposer de toutes les fonctionnalités de Craft, y compris la gestion d'utilisateurs, une API GraphQL et la capacité de modifier le branding de l'outil.
...
Si vous en avez besoin, vous pouvez ensuite payer 99 USD à n'importe quel moment pour avoir droit à un an de mises à jour et de support.

#### Versions de test

Vous pouvez tester Craft Pro et toutes ses fonctionnalités gratuitement pourvu que vous soyez sur un domaine local et identifiable comme tel.
...
- [L'utilisation de Twig comme language de templating](https://twig.symfony.com/): cela induit un temps d'apprentissage mais les gains en termes de puissance, de modularité et de flexibilités sont importants.
...
Après avoir vérifié que votre serveur est capable de faire tourner Craft, il ne vous reste qu'à suivre la [procédure d'installation](https://craftcms.com/docs/5.x/install.html) et à vérifier l'ensemble de vos permissions de dossiers et de fichiers. Cela devrait être assez rapide.

### Version control

La plupart des développeuses / développeurs utilisent Git pour maintenir leur code et en gérer les différentes versions. Craft possède son propre fichier `.gitignore`. Il vous reste à gérer vos propres fichiers et dossiers. Voici le `.gitignore` que j'utilise comme base.
...
Ces sections contiennent une seule entry. Elles sont utilisées pour les pages particulières de votre site, comme par exemple une page "about" ou votre "homepage".
...
- les entry types et les champs associés pour créer la data structure de l'entry de cette section.
...
- les entry types et les champs associés pour créer la data structure des entries de cette section.
...
Les entry types peuvent être utilisés [dans vos tags `craft.entries()`](https://craftcms.com/docs/5.x/reference/element-types/entries.html#type) et vos tests conditionnels.
...
Dans Craft, les [utilisateurs](https://craftcms.com/docs/5.x/system/user-management.html) servent à gérer les permissions données aux divers utilisateurs du système.
...
L'une des grandes forces de Craft c'est qu'il est possible très facilement de créer des [relations](https://craftcms.com/docs/5.x/system/relations.html) entre Entries, Users et Assets.
...
Pour exploiter ces relations dans vos templates, Craft met à votre disposition un outil extrêmement puissant: le paramètre [`relatedTo`](https://craftcms.com/docs/5.x/system/relations.html#the-relatedto-parameter), utilisable avec `craft.entries()`, `craft.users()` et `craft.assets()`.

### Routing

L'un des autres éléments intéressant de Craft c'est le [routing](https://craftcms.com/docs/5.x/system/routing.html) dynamique, qui permet de séparer structure des URLs et architecture de dossiers et de fichiers.
...
Il est possible créer des routes et de spécifier quel template doit être chargé par Craft. Un exemple facile à comprendre est [un template donnant accès à une archive des entries classées par année](https://craftcms.com/docs/5.x/system/routing.html#creating-routes).

En créant une route dynamique `blog/archive/{year}` renseignant le template `blog/archive`, les URL `blog/archive/2013` et `blog/archive/2012` vont charger le même template et rendre disponible une variable `year` utilisable par Twig et par Craft avec les paramètres [`after`](https://craftcms.com/docs/5.x/reference/element-types/entries.html#after) et [`before`](https://craftcms.com/docs/5.x/reference/element-types/entries.html#before).

Si vous avez besoin de plus de possibilités que celles offertes par le control panel, sachez que [les routes peuvent également être gérées via un fichier](https://craftcms.com/docs/5.x/system/routing.html#advanced-routing-with-url-rules) `config/routes.php`, ce qui vous donne accès à du matching d'URL en utilisant des expressions régulières.

## 3. Twig comme language de templating

Craft utilise [Twig](https://twig.symfony.com/), créé par Fabien Potencier pour Symfony, comme language de templating. Twig a l'avantage de compiler les templates en PHP, ce qui lui permet d'être très performant. C'est un language qui reste également assez [accessible pour des développeurs front-end](https://twig.symfony.com/doc/3.x/templates.html), même si une période d'apprentissage existe.

Couplé à des tags, fonctions et filtres spécifiques à Craft, Twig vous permet de récupérer et de manipuler vos données au sein de vos templates. Pour une introduction rapide à Twig, vous pouvez [consulter les slides de Brandon Kelly sur Slideshare](https://www.slideshare.net/slideshow/twig-for-designers/36103036).

### Principaux tags dans Twig

En plus des [tags disponibles dans Twig](https://twig.symfony.com/doc/3.x/tags/index.html), Craft possède également quelques tags qui lui sont propres. Nous reviendrons sur certains d'entre-eux dans la suite du cours.
...
Créer une variable `allEntries` à laquelle est assignée le résultat d'une [ElementQuery](https://craftcms.com/docs/5.x/development/element-queries.html) contenant les 10 dernières entries dans la section blog, classées par date de création en ordre descendant.
...
Créer une [structure de contrôle](https://twig.symfony.com/doc/3.x/templates.html#control-structure) vérifiant si la variable `allEntries` contient au minimum une entry.
...
Les [fonctions disponibles dans Twig](https://twig.symfony.com/doc/3.x/functions/index.html) permettent de produire et de manipuler des contenus. Craft possède également [ses propres fonctions](https://craftcms.com/docs/5.x/reference/twig/functions.html), en plus de celles fournies par Twig.
...
`dump()` est une fonction extrêmement utile disponible lorsque [Dev Mode](https://craftcms.com/knowledge-base/what-dev-mode-does) est activé. Utile pour le debugging.
...
Twig est capable d'interpréter toutes sortes d'[opérations mathématiques](https://twig.symfony.com/doc/3.x/templates.html#math) et de manipuler des chaînes de caractères (strings) à l'aide de [filtres](https://twig.symfony.com/doc/3.x/filters/index.html).
...
[Les Macros](https://twig.symfony.com/doc/3.x/tags/macro.html) dans Twig sont comparables à des mixins en Sass. Pensez à elles comme à de petits blocs de code réutilisables.
...
Avec Craft, vous interagissez avec la base de données en utilisant des [Element Queries](https://craftcms.com/docs/5.x/development/element-queries.html). Cela à l'air très compliqué mais c'est en fait un concept assez simple:
...
4. Craft retourne un element ou un array d'elements ([entry](https://docs.craftcms.com/api/v5/craft-elements-entry.html), [user](https://docs.craftcms.com/api/v5/craft-elements-user.html), [asset](https://docs.craftcms.com/api/v5/craft-elements-asset.html)).
...
[`craft.entries()`](https://craftcms.com/docs/5.x/reference/element-types/entries.html#querying-entries) est le tag que vous allez utiliser pour récupérer vos entries.
...
2. D'accéder à une série de [méthodes permettant de les manipuler](https://laravel.com/docs/11.x/collections#available-methods) héritées de Laravel.
...
Il est possible de créer des Element Queries plus complexes en utilisant des [Element Queries avancées](https://craftcms.com/docs/5.x/development/element-queries.html#advanced-element-queries).

#### Pas de résultats

Vous pouvez facilement afficher un contenu alternatif si aucun résultat n'est trouvé en [utilisant une clause {% else %} dans votre loop](https://twig.symfony.com/doc/3.x/tags/for.html#the-else-clause).
...
Les [autres tests disponibles avec Twig](https://twig.symfony.com/doc/3.x/tests/index.html) valent également la peine d'être consultés, notamment le test `is defined`. Les tests disponibles dans Twig sont:
...
- Vérifier que l'option `preloadSingles()` est activée dans votre fichier `config/general.php` (c'est le cas par défaut) et accéder directement au contenu du champs via `{{ companyInfo.companyName }}`.
...
`craft.users()` permet d'accéder aux utilisateurs de votre site. Ce tag possède lui aussi [un certain nombre de paramètres, dont certains lui sont propres](https://craftcms.com/docs/5.x/reference/element-types/users.html).

Son fonctionnement est semblable au tag `craft.entries()` mais il retourne un objet ou un array d'objets [`User`](https://docs.craftcms.com/api/v5/craft-elements-user.html).

### Assets et transformations

`craft.assets()` permet d'accéder aux Assets de votre site. Ce tag possède lui aussi [un certain nombre de paramètres, dont certains lui sont propres](https://craftcms.com/docs/5.x/reference/element-types/assets.html#parameters).

Son fonctionnement est semblable au tag `craft.entries()` dans la mesure où il retourne un objet [`Asset`](https://docs.craftcms.com/api/v5/craft-elements-asset.html).
...
[Matrix](https://craftcms.com/docs/5.x/reference/field-types/matrix.html) est un type de champ particulièrement intéressant et puissant dont vous disposez dans Craft. Matrix vous permet de combiner et d'arranger différents entry types (et leurs champs liés) au sein d'un champ.

Vous pourriez par exemple créer un champ Matrix `modularBody` avec la configuration suivante:
...
Craft possède un [système de recherche très puissant](https://craftcms.com/docs/5.x/system/searching.html) basé sur un paramètre `search` utilisable avec les tags `craft.entries()`, `craft.users()`, `craft.assets()` et `craft.tags()`.
...
Si vous essayez d'avoir une entry (single) pour chaque page de votre site, Craft permet facilement de disposer d'une navigation principale flexible et pouvant même être mise à jour par les éditeurs du site. J'utilise généralement une section de type structure pour fournir ce genre de fonctionnalité.
...
Les sites multilingues sont en général assez complexes. Craft rend les choses plus faciles dans la mesure où le multilingue est géré nativement. Les bases sont assez simples et sont détaillée dans la [section Localization](https://craftcms.com/docs/5.x/system/sites.html) de la documentation.
...
"[Eager-loading](https://craftcms.com/docs/5.x/development/eager-loading.html)" est une façon d'indiquer à Craft que lorsqu'il va chercher ces entries, chacune d'entre elles possède également un asset qu'il faudra aller chercher lui aussi. Une fois prévenu, Craft va aller chercher les entries et les assets liés en utilisant le moins de queries MySQL possible.

Pour faire de l'eager loading, il vous faudra utiliser `eagerly()` dans vos queries imbriquées ou utiliser le paramètre `with` dans votre query principale.
...
- single section
- affiche les 4 derniers case studies (channel section)
...
- [Documentation Twig](https://twig.symfony.com/) pour les template designers
